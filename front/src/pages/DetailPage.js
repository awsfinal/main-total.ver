...
function DetailPage() {
  ...
  useEffect(() => {
    console.log('DetailPage location.state:', location.state);
    
    // location.state에서 GPS 데이터가 전달된 경우 (카메라에서 온 경우)
    if (location.state && location.state.gpsData) {
      console.log('✅ GPS 데이터 수신 성공:', location.state.gpsData);
      console.log('📍 위도:', location.state.gpsData.latitude);
      console.log('📍 경도:', location.state.gpsData.longitude);
      setGpsData(location.state.gpsData);
    } else {
      console.log('❌ GPS 데이터 수신 실패 - location.state:', location.state);
    }
  ...
  useEffect(() => {
    console.log('=== DetailPage useEffect 실행 ===');
    console.log('gpsData 존재:', !!gpsData);
    console.log('gpsData 내용:', gpsData);
    
    if (gpsData && gpsData.latitude && gpsData.longitude) {
      console.log('🔄 GPS 데이터 유효, 1초 후 지도 초기화');
      
      const timer = setTimeout(() => {
        console.log('🕰️ 1초 경과, 지도 초기화 시작');
        initKakaoMap();
      }, 1000);
  ...
  const initKakaoMap = async () => {
    ...
    try {
      ...
      var options = {
        center: new kakao.maps.LatLng(gpsData.latitude, gpsData.longitude),
        level: 3
      };
      ...
      var markerPosition = new kakao.maps.LatLng(gpsData.latitude, gpsData.longitude);
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                <span style={{ fontSize: '20px' }}>📍</span>
                <span style={{ fontSize: '16px', fontWeight: 'bold', color: '#333' }}>내 위치</span>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '14px', color: '#007AFF', fontWeight: 'bold' }}>
                    위도: {gpsData.latitude ? gpsData.latitude.toFixed(7) : 'N/A'}, 경도: {gpsData.longitude ? gpsData.longitude.toFixed(7) : 'N/A'}
                  </span>
                </div>
                ...
                {gpsData.heading && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '14px', color: '#333' }}>
                      방위: {gpsData.heading.toFixed(1)}° ({gpsData.headingSource})
                    </span>
                  </div>
                )}
